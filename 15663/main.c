...
void GetAnswer(int select_num, int select_num_max, int max_index, int serial_index)
{
	if (select_num == 0)
	{
	...
	else
	{
		...
		{
			if (used[i] == false && last_num != nums[i])
			{
				serial[serial_index] = nums[i];
				last_num = nums[i];
				used[i] = true;
				GetAnswer(select_num - 1, select_num_max, max_index, serial_index + 1);
				used[i] = false;
			}
		}
	}
